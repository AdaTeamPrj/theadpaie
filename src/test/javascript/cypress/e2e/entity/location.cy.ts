...
describe('Location e2e test', () => {
  ...
  const locationSample = {
    streetName: "Lucie d'Abbeville",
    numeroRue: 'cyan',
    postalCode: 'Tasty Home connecting',
    city: 'Lake Julieton',
    stateProvince: 'Account JSON',
  };
  ...
  describe('new Location page', () => {
    ...
    it('should create an instance of Location', () => {
      cy.get(`[data-cy="streetName"]`).type('Picard d&#39;Alésia').should('have.value', 'Picard d&#39;Alésia');

      cy.get(`[data-cy="numeroRue"]`).type('programming Practical').should('have.value', 'programming Practical');

      cy.get(`[data-cy="postalCode"]`).type('a').should('have.value', 'a');

      cy.get(`[data-cy="city"]`).type('Lake Ouryland').should('have.value', 'Lake Ouryland');
